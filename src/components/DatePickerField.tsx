...
export function DatePickerField({ value, onChange, accent = "blue" }: Props) {
  ...
  const accentBg = accent === "gold" ? "bg-saldio-gold" : "bg-saldio-blue";
  const accentText = accent === "gold" ? "text-saldio-gold-deep" : "text-saldio-blue";
  const accentSoftBg = accent === "gold" ? "bg-saldio-gold-bg" : "bg-saldio-sky";
  ...
  return (
    <View>
      ...
      <View className="mb-2 flex-row gap-2">
        {[
          ...
          <Pressable
            ...
            className={`rounded-full px-4 py-2 ${value === opt.iso ? accentSoftBg : "bg-white"}`}
          >
            <Text
              className={`font-sans-semibold text-xs ${
                value === opt.iso ? accentText : "text-saldio-soft"
              }`}
            >
              {opt.label}
            </Text>
      ...
      <Pressable
        onPress={openPicker}
        className="flex-row items-center gap-3 rounded-2xl bg-white px-4 py-3.5 active:opacity-80"
      >
        <Ionicons name="calendar" size={18} color={accentColor} />
        <Text className="flex-1 font-sans-semibold text-base text-saldio-ink">
      ...
      <Modal visible={open} transparent animationType="fade" onRequestClose={() => setOpen(false)}>
        <Pressable className="flex-1 justify-end bg-black/40" onPress={() => setOpen(false)}>
          <Pressable className="mx-4 mb-6 rounded-3xl bg-white p-5" onPress={() => {}}>
            {/* Navigasi bulan */}
            <View className="mb-4 flex-row items-center justify-between">
              <Pressable
                onPress={() => shiftMonth(-1)}
                className="h-10 w-10 items-center justify-center rounded-full bg-saldio-bg active:opacity-70"
              >
                <Ionicons name="chevron-back" size={18} color="#101736" />
              </Pressable>
              <Text className="font-sans-bold text-base text-saldio-ink">
              ...
              <Pressable
                ...
                className="h-10 w-10 items-center justify-center rounded-full bg-saldio-bg active:opacity-70"
              >
                <Ionicons name="chevron-forward" size={18} color="#101736" />
              </Pressable>
            </View>
            ...
            <View className="mb-1 flex-row">
              {WEEKDAYS_MIN.map((d) => (
                <View key={d} className="flex-1 items-center py-1">
                  <Text className="font-sans-medium text-[11px] text-saldio-muted">{d}</Text>
                </View>
              ))}
            </View>
                      ...
                      >
                        <Text
                          className={`font-sans-semibold text-sm ${
                            isSelected ? "text-white" : isToday ? accentText : "text-saldio-ink"
                          }`}
                        >
                          {day}
                        </Text>
